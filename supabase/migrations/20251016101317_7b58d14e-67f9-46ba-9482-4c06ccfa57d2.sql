...
-- Create mood_test_results table
CREATE TABLE public.mood_test_results (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  score INTEGER NOT NULL CHECK (score >= 0 AND score <= 30),
  level TEXT NOT NULL CHECK (level IN ('low', 'mild', 'moderate', 'severe')),
  answers JSONB NOT NULL,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Enable RLS on mood_test_results
...
-- RLS Policies for mood_test_results
CREATE POLICY "Users can view own test results"
  ON public.mood_test_results
  FOR SELECT
  TO authenticated
  USING (auth.uid() = user_id);
